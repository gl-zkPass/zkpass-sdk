 ...
 */
use crate::interface::{
    DataVerificationRequest, KeysetEndpoint, PublicKey, PublicKeyOption, UserDataRequest,
    VerifiedNestedTokenDvr, ZkPassError,
};
...
pub fn decode_dvr_token(dvr_jwt: &str) -> Result<(DataVerificationRequest, Header), ZkPassError> {
    ...
    match decode::<WrappedDataVerificationRequest>(dvr_jwt, &decoding_key, &validation) {
        ...
        Err(err) => Err(ZkPassError::CustomError(format!(
            "MissingRootDataElementError: {err}"
        ))),
    }
}
...
pub fn verify_dvr_nested_token(
    ...
    let dvr: DataVerificationRequest = serde_json::from_value(dvr_payload).map_err(|e| {
        ZkPassError::CustomError(format!(
            "Error parsing payload to DataVerificationRequest: {}",
            e.to_string()
        ))
    })?;
...
fn validate_user_data_requests(
    ...
    if user_data_requests.is_empty() {
        return Err(ZkPassError::QueryEngineError(String::from(
            "User data requests must not be empty",
        )));
    }
...
mod test {
    ...
    fn get_mock_dvr() -> DataVerificationRequest {
        DataVerificationRequest {
            dvr_verifying_key: Some(PublicKeyOption::PublicKey(PublicKey {
                x: String::from("x"),
                y: String::from("y"),
            })),
    ...
    fn get_encoded_token<T>(data: T) -> String
    where
        T: Serialize,
    {
        jsonwebtoken::encode(
    ...
    fn test_validate_user_data_requests_success() {
        let multi_user_data_requests = hashmap! {
            String::from("user_data_1") => mock_user_data(),
            String::from("user_data_2") => mock_user_data(),
        };
        let single_user_data_requests = hashmap! {
            String::from("") => mock_user_data(),
        };

        assert_eq!(
            validate_user_data_requests(&multi_user_data_requests).unwrap(),
            true
        );
        assert_eq!(
            validate_user_data_requests(&single_user_data_requests).unwrap(),
            true
        );
    }
    ...
    fn test_validate_user_data_requests_empty_key() {
        let user_data_requests = hashmap! {
            String::from("") => mock_user_data(),
            String::from("user_data_2") => mock_user_data(),
        };
    ...
    fn test_get_dvr_public_key_option() {
        ...
        dvr.dvr_verifying_key = Some(PublicKeyOption::KeysetEndpoint(KeysetEndpoint {
            jku: String::from("jku"),
            kid: String::from("kid"),
        }));
        assert_eq!(
    ...
    fn test_get_keyset_endpoint_params() {
        ...
        assert_eq!(
            get_keyset_endpoint_params(&header).unwrap(),
