...
use super::{
    interface::{ VerifiedNestedTokenUserData, ZkPassError },
...
pub fn verify_user_data_nested_token(
    ...
    let jws_tokens = decode_user_data_tokens(&unwrapped_jwe_payload).map_err(|e|
        ZkPassError::CustomError(e.to_string())
    )?;
    ...
    for (tag, jws_token) in jws_tokens.iter() {
        let verifying_key = verifying_keys.get(tag).ok_or(ZkPassError::MissingPublicKey)?;
