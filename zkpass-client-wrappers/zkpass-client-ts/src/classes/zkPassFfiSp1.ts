...
export class ZkPassFfiSp1 extends ZkPassFfi {
  private zkPassQueryFfiSp1 = ffi.Library(
    resolveModulePath("libsp1_zkpass_query.so"),
    {
  ...
  verifyZkProof(receipt: string): ZkPassOutput {
    const output: string = this.catchError(
      this.zkPassQueryFfiSp1.sp1_verify_zkproof_wrapper(receipt)
    );
  ...
  getQueryEngineVersion(): string {
    return this.catchError(
      this.zkPassQueryFfiSp1.sp1_get_query_engine_version_wrapper()
    )!;
  }

  getQueryMethodVersion(): string {
    return this.catchError(
      this.zkPassQueryFfiSp1.sp1_get_query_method_version_wrapper()!
    );
  }
}
