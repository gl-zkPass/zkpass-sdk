...
export default function EmployeeOnboarding({
  ...
  const _getDVRPayload = async (dvrJwt: string) => {
    ...
    const JWKS = jose.createRemoteJWKSet(
      new URL("http://localhost:3009/verifier/jwks.json")
    );
    // const mockJWKS = jose.createRemoteJWKSet(
    //   new URL(
    //     "https://raw.githubusercontent.com/zulamdat/zulamdat.github.io/sample-key/zkp-key/verifier-key.json"
    //   )
    // );

    const mockJWKS = jose.createLocalJWKSet({
      keys: [
        {
          kty: "EC",
          x: jose.base64url.encode(
    ...
    // const { payload, protectedHeader } = await jose.jwtVerify(dvrJwt, mockJWKS);
  ...
  const _getBloodTestPayload = async (bloodTestJwt: string) => {
    console.log("_getBloodTestPayload before if");

    if (bloodTestJwt.length === 0) {
      return;
    }
    console.log("_getBloodTestPayload");
    const JWKS = jose.createRemoteJWKSet(
      new URL("http://localhost:3009/issuer/jwks.json")
    );
